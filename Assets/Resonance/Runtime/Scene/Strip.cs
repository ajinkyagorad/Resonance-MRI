...
namespace Nebulytic.Resonance
{
    ...
    {
        public const float W = 0.72f, H = 0.152f;
        ...
        const float TrackX0 = -0.178f, TrackW = 0.2f, RowY = -0.05f;
        ...
        readonly Transform fill; readonly List<GameObject> ticks = new List<GameObject>();
        ...
        public Strip(App app, Transform parent)
        {
            ...
            title = Text("Title", new Vector3(-inner + 0.13f, 0.052f, -0.002f), 0.3f, 0.02f, 0.13f, TextAlignmentOptions.Left);
            title.color = Look.TEXT2;
            clock = Text("Clock", new Vector3(inner - 0.13f, 0.052f, -0.002f), 0.3f, 0.02f, 0.12f, TextAlignmentOptions.Right);
            clock.color = Look.TEXT3;
            caption = Text("Subtitles", new Vector3(0, 0.004f, -0.002f), W - 0.12f, 0.062f, 0.2f, TextAlignmentOptions.Center);
            caption.color = Look.TEXT;
            caption.textWrappingMode = TextWrappingModes.Normal; caption.overflowMode = TextOverflowModes.Overflow;
            // Progress: one line.
            Mats.Object("Track", Root, MeshKit.RoundedPanel(TrackW, 0.004f, 0.002f, 0.001f, 3), Mats.Flat(Look.TRACK * 1.6f), new Vector3(TrackX0 + TrackW / 2, RowY, -0.002f));
            fill = Mats.Object("Fill", Root, MeshKit.Quad(1, 0.004f), Mats.Flat(Look.TEXT), new Vector3(TrackX0, RowY, -0.003f)).transform;
            // Transport icons (one side) and toggle chips (the other), about 1.7 times 0.8.3's.
            var icon = Mats.Flat(Look.TEXT);
            float ix = -inner + 0.006f; const float iconScale = 1.7f;
            Button("prev", new Vector3(ix, RowY, -0.003f), TriangleMesh(false), icon).transform.localScale = Vector3.one * iconScale;
            ...
            float cx = inner - 0.232f; const float dx = 0.052f;
            ...
            Mats.Object("Card", creditsCard.transform, MeshKit.RoundedPanel(0.78f, 0.3f, 0.024f, 0.004f), panelMat, new Vector3(0, 0, 0.004f));
            credits = Text("Credits text", new Vector3(0, 0, -0.002f), 0.74f, 0.27f, 0.11f, TextAlignmentOptions.TopLeft, creditsCard.transform);
            ...
            var col = Root.gameObject.AddComponent<BoxCollider>(); col.size = new Vector3(W, H, 0.02f); col.isTrigger = true;
        ...
        /// <summary>No cue ticks since 0.8.3 (a plain progress line); kept for callers.</summary>
        ...
        public void Tick(int stepIndex, int stepCount, LessonStep step, LessonCue cue, double tau, bool playing, bool overview, string status, double t, double speed, bool lab)
        {
            ...
            fill.localScale = new Vector3(TrackW * f, 1, 1); fill.localPosition = new Vector3(TrackX0 + TrackW * 0.5f * f, RowY, -0.003f);
